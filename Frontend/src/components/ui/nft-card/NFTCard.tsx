...
/**
 * Lightweight Image component used instead of next/image to avoid Next.js dependency in environments
 * that don't provide next/image typings. Supports the common props used by this file.
 */
...
}> = ({ src, alt, fill, width, height, style, onLoad, onError, className, onClick }) => {
  const handleLoad = () => {
    onLoad?.();
  };
  const handleError = () => {
...
export const NFTCard: React.FC<NFTCardProps> = ({
  ...
  // pull price from props if provided via intersection typing
  // (we avoid changing the core NFT type; price is optional on the card props)
          ...
          <div className={styles.infoSection}>
            {priceWei && (
              <div className="mb-3">
                ...
                <p className="text-2xl font-bold text-green-600 dark:text-green-400">{weiToEth(priceWei)} ETH</p>
                <p className="text-xs text-gray-500 mt-1">{weiToGwei(priceWei)} Gwei • {priceWei} Wei</p>
              </div>
            )}
      ...
      <div className="p-4 flex-1 flex flex-col">
        ...
        {priceWei && (
          <div className="mt-3">
            ...
            <p className="text-lg font-semibold text-green-600 dark:text-green-400">{weiToEth(priceWei)} ETH</p>
            <p className="text-xs text-gray-500 mt-1">{weiToGwei(priceWei)} Gwei</p>
          </div>
        )}
      </div>
    </div>
